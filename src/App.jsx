...
function App() {
  ...
  return (
    <div className="App">
      ...
      {
        myObjects.map(myObjects => <Card type="Object" title={myObjects.title}/>)
      }
      {
        myContexts.map(myContexts => <Card type="Context" title={myContexts.title} />)
      }
      {
        myEvents.map(myEvents => <Card type="Type of Future" title={myEvents.title} description={myEvents.description}/>)
      }
      {
        myMoods.map(myMoods => <Card type="Emotion" title={myMoods.title} />)
      }
      </div>
      <h1>Let's think about mobility!</h1>
      <div className="cardGroup">
      {
        myCarObjects.map(myCarObjects => <Card type="Object" title={myCarObjects.title}/>)
      }
      {
        myCarContexts.map(myCarContexts => <Card type="Context" title={myCarContexts.title} />)
      }
      {
        myCarEvents.map(myCarEvents => <Card type="Type of Future" title={myCarEvents.title} description={myCarEvents.description}/>)
      }
      {
        myCarMoods.map(myCarMoods => <Card type="Emotion" title={myCarMoods.title} />)
      }
      </div>
    </div>
